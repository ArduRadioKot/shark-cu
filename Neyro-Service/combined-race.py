...
- Combined: head tilt turning + forward/backward drive from progress
"""
...
TARGET_SERIAL = '821733' # Серийник нейроинтерфейса для подключения, например "821619"
...
THRESHOLD_ALPHA_INIT = 1.8e-11  # From fix.py
ACCUM_STEP_SEC = 0.1
ACCUM_STEP = 1
...
HEAD_TILT_THRESHOLD = 0.3  # Increased from 0.25 to reduce sensitivity
...
class BleAirshipController:
    ...
    async def _connect(self):
        ...
        await asyncio.sleep(0.5)  # Allow service discovery to complete
    ...
    async def _close(self):
        ...
        if self._client is not None and self._client.is_connected and self._write_lock is not None:
            async with self._write_lock:
...
def _airship_cmd(direction: str):
    # Commands from fix.py for forward/backward
    if direction == "F":
        return "AA813"
    if direction == "B":
        return "AA714"
...
def drive_logic_from_progress(direction):
    # From fix.py - simple forward/backward control
    # Priority: if turning (head tilt), skip forward/backward movement
    if direction is None:
        return
    if _drive_enabled_at is None or time.time() < _drive_enabled_at:
        return

    # Skip forward/backward if currently turning
    if head_tilt_turn_state != "S":
        return
...
def send_head_tilt_turn_command(state: str):
    # From povorot-turn.py - correct turning commands
    if state == "R":
        _send_cmd("AA615")  # Right turn
    elif state == "L":
        _send_cmd("AA516")  # Left turn
    else:
        _send_cmd("AA507")  # Stop
...
def on_mems(mems: MEMS, md: MEMSTimedData):
    # From povorot-turn.py - head tilt turning control
...
fig.canvas.manager.set_window_title("CU_NeuroRace_Combined")  # название окна
...
def update_plot(_frame):
    ...
    # Progress bar + drive logic from progress (from fix.py)
...
def main():
    ...
    print("CU_NeuroRace Combined Dashboard запущен!")
    print("Controls:")
    print("  - Forward/Backward: from alpha progress (alpha > threshold)")
    print("  - Turning: from head tilt (MEMS)")
    ...
    try:
        ...
        running = False
        cleanup()
        print("CU_NeuroRace Combined остановлен.")
